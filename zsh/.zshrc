...
ai-web() {
    ...
    if ! pgrep -f "open-webui" > /dev/null 2>&1; then
        ...
        DATA_DIR="$HOME/.local/share/open-webui" open-webui serve \
            &> /tmp/open-webui.log &
        disown

        # Aguarda o servidor responder de verdade, até 30s
        echo -n "Aguardando Open WebUI..."
...
ai-stop() {
    pkill -f "open-webui" && echo "Open WebUI encerrado."
    sudo systemctl stop ollama && echo "Ollama encerrado."
}

alias update='sudo pacman -Syu && uv tool upgrade open-webui'

# ─── FUNÇÕES: FIREWALL ───────────────────────────────────────
firewall_ssh() {
    if firewall-cmd --zone=public --query-service=ssh-custom >/dev/null 2>&1; then
        echo "Desativando SSH na porta 3773..."
        sudo firewall-cmd --zone=public --remove-service=ssh-custom
        sudo systemctl stop sshd
    else
        echo "Ativando SSH na porta 3773..."
        sudo firewall-cmd --zone=public --add-service=ssh-custom
...
firewall_python() {
    if firewall-cmd --zone=public --query-service=python-http >/dev/null 2>&1; then
        echo "Fechando porta 8000..."
        sudo firewall-cmd --zone=public --remove-service=python-http
    else
        echo "Abrindo porta 8000 para Lab..."
        sudo firewall-cmd --zone=public --add-service=python-http
    fi
    firewall-cmd --zone=public --list-services
}
